...
export function ClassBooking({ setPage }: { setPage?: (p: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/*
        ══════════════════════════════════════════════════════════════════
        OCTIV BANNER — running alongside in-app PayFast, by design
        ══════════════════════════════════════════════════════════════════
        Boss (Siyabonga) wants both non-member payment paths visible for
        now: the client hasn't yet decided whether to fully commit contract
        billing to PayFast, so Octiv stays up as the safety net alongside
        the normal in-app "Pay R__ →" button (via initiatePayFastForClass)
        on each class card below. Neither path is being deprecated right
        now — this is a deliberate "keep both live" state, not a bug
        workaround.

        WHEN THE CLIENT FINALLY COMMITS TO PAYFAST-ONLY:
          Delete this whole banner block (between the markers) — nothing
          else changes. The in-app PayFast button already works standalone.

        Do NOT delete the Octiv link on the Dashboard's DropInCard — that
        one is permanent, for trial/drop-in visitors who aren't registering
        as members at all.
        ══════════════════════════════════════════════════════════════════
      */}
      {!isMember && (
        ...
        >
          <div>
            <div className="font-bold text-sm">
              💳 Prefer to book via Octiv?
            </div>
            <div className="text-xs text-muted-foreground mt-0.5">
              You can also book &amp; pay for non-member classes directly
              through Octiv.
            </div>
          </div>
          <a
          ...
          {dayClasses.map((cls, i) => {
            ...
            const handleBook = async () => {
              if (isMember || !chargeable) {
                ...
                // non-member classes. It runs alongside the Octiv banner
                // above — both paths are intentionally live right now.
